  ...
  const animateEntrance = useCallback(
    (direction) => {
      ...
      if (slideIndex === 1) {
        ...
        gsap.set([label, headline, body, profileFrame].filter(Boolean), {
          clearProps: "all",
        });
      ...
      if (slide.id.startsWith("slide-section-")) {
        ...
        const productStamp = slide.querySelector(
          ".section-cover-product-stamp",
        );
        const logoStamp = slide.querySelector(".section-cover-logo-stamp");

        gsap.set([numEl, titleEl, productStamp, logoStamp].filter(Boolean), {
